...
double getMemoryUsage()
{
	...
	output  = popen("free -m | awk '/Mem:/ {print$2}'","r");	
	fscanf(output, "%d", &memoryTotal);
	pclose(output);	

	output  = popen("free -m | awk '/Mem:/ {print$3}'","r");
	fscanf(output, "%d", &memoryUsed);
 	pclose(output);
	free(output);

	return (memoryUsed / (double)memoryTotal);
}

static void push_data_to_all_websocket_connections(struct mg_mgr *m) {
  struct mg_connection *c;
  int memory_usage = (double) getMemoryUsage() * 100.0;// rand() / RAND_MAX * 100.0;
